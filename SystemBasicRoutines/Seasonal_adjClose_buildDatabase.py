...
from Basic_Seasonal_adjClose import calc_weekly_data, calc_monthlydata
from Basic_Seasonal_adjClose import getdata, check_winning_period, check_loosing_period
...
def build_seasonal_db(target__dir='..\\output\\', ticker=""):
    ...
    with pd.ExcelWriter(target__dir + '\\SeasonalCheck_adjClose.xlsx') as writer:
        df_result_w.to_excel(writer, sheet_name='Upside')
        df_result_l.to_excel(writer, sheet_name='Downside')

    print(f"File: " + target__dir + '\\SeasonalCheck_adjClose.xlsx')
...
print(f"Seasonal Database finished - time: {(time.time() - starttime)} Seconds")
